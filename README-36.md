...
- **Stratified** on `Survived` so both splits preserve the 38.4 % base rate.
- **Hold-out test set never touches preprocessing or tuning** — every fitted transformer (StandardScaler, OneHotEncoder) lives inside the sklearn `Pipeline`, so it is re-fit on the training fold only. **Leakage is structurally impossible.**
- **Hyperparameters tuned on the training set** with 5-fold `StratifiedKFold` cross-validation; the test set is opened **once** at the end.

---

## Modelling

A `Pipeline` of `ColumnTransformer(StandardScaler + OneHotEncoder) → LogisticRegression`, tuned with `GridSearchCV` over `C ∈ {0.001, 0.01, 0.05, 0.1, 0.5, 1, 5, 10}` and `penalty ∈ {l1, l2}`, scoring on F1.

**Best parameters:** `C = 0.1`, `penalty = l2`. Stronger regularisation won — exactly as expected on 891 rows where unregularised LR would over-weight rare title combinations.

### Model comparison (5-fold CV)

![Model comparison](images/02_model_comparison.png)

Four candidates were benchmarked: a `Dummy` baseline, `LogisticRegression`, `RandomForest`, and `GradientBoosting`. On 891 rows, LR and RF are statistically indistinguishable; LR was chosen for **interpretable coefficients and exact SHAP** rather than approximate tree explanations.
...
Bootstrap 95 % CI on ROC-AUC: **(0.79, 0.89)** — the AUC is statistically distinct from 0.5 at the 95 % level.
...
![ROC PR](images/04_roc_pr_curves.png)

### Calibration

![Calibration](images/05_calibration.png)

The reliability curve sits close to the diagonal; the Brier score of **0.142** confirms the predicted probabilities are trustworthy — important if the model is ever used to rank passengers by risk rather than just classify them.

### Learning curve — bias / variance diagnostic

![Learning curve](images/06_learning_curve.png)

Training and validation curves converge near 0.78 F1, with a small remaining gap. **More data would help only marginally** — the dataset is approaching its irreducible noise floor (the *Bayes error rate* for this problem).

### Operational threshold tuning

![Threshold tuning](images/07_threshold_tuning.png)

For an evacuation-style task, missing a real survivor is worse than a false alarm. **F2** weights recall 4× higher than precision; the F2-optimal threshold differs from the F1-optimal threshold and is the right setting if the model were ever deployed for triage.
...
| `HasCabin` | +0.9 | second class signal |
...
A model-agnostic check that confirms the coefficient story: shuffling `Sex` or `Title` collapses F1 the most.

---

## Where statistics meets story — three named passengers

Per the mentor's brief, three real passengers were inspected with **exact SHAP for linear models** (no `shap` package needed; `φᵢ(x) = βᵢ · zᵢ(x)` and the test `test_linear_shap_sums_to_log_odds` proves the reconstruction is exact to 1e-10).

### Mr. Thomas Andrews — ship's designer (1st class male, age 39)

![Andrews](images/10_shap_andrews.png)

Actual outcome: **DIED.** Model prediction: ~0.91 survival.

The model sees `Pclass=1, has cabin, age 39`. The model cannot see that Andrews designed the *Titanic*, walked the ship telling passengers to put on lifejackets, and gave away his lifeboat seat. **The residual is ethics, not error.**

### Miss Millvina Dean — youngest passenger (3rd class infant, 0.17 yrs)

![Dean](images/11_shap_dean.png)

Actual outcome: **SURVIVED.** Model prediction: ~0.81 survival.

`Sex=female` and the infant age push survival probability up enough to overcome the 3rd-class penalty. The model learned a social norm — *children first* — that was never officially in the policy. That is the most striking thing in this whole project.

### Mrs. Margaret 'Molly' Brown (1st class female, age 44)

![Brown](images/12_shap_brown.png)

Actual outcome: **SURVIVED.** Model prediction: ~0.72 survival.

The age penalty pulls the prediction below the population's first-class-female average. Brown survived because she threatened a lifeboat officer with an oar to make him return for swimmers. **The residual is agency — there is no `grit` feature.**

---

## What the errors actually contain

The **18 % error rate is not random noise.** It contains the human interest in the dataset — the false positives are 1st-class men who helped others board lifeboats; the false negatives are 3rd-class women who escaped through luck or a deck hatch. The statistics describe groups; the errors describe people.
...
├── tests/test_pipeline.py                # 13 sanity tests (incl. SHAP additivity)
├── requirements.txt
├── README.md
├── .gitignore
├── Titanic Survival prediction final.pdf # Mentorship report (v2)
├── images/                                # Charts (committed — render in README)
│   ├── 00_banner.png
│   ├── 01_eda_overview.png
...
All 13 PNGs land in `images/`, model artefacts in `outputs/`. Running takes 30–60 s on Termux on a recent Android phone.
...
| Question | Answer |
|---|---|
| **Why Logistic Regression and not XGBoost?** | XGBoost beats LR by ~1 % on 891 rows — well inside CV variance. LR gives signed coefficients you can read in plain English. Choose interpretability. |
...
| **Why drop the cabin letter and keep `HasCabin`?** | The cabin letter is 77 % missing. The fact of having a cabin is a clean binary class signal — kept. |
| **Why median impute Age by `Pclass × Sex`, not KNN?** | KNN would condition Age on features that correlate with the target — that is a soft form of leakage. |
| **Where is leakage prevented?** | Every fitted transformer lives inside the sklearn `Pipeline`, so it is re-fit on the training fold of every split. Leak-free by construction. |
| **Why does the SHAP plot not need the `shap` package?** | For a linear model with standardised inputs, `φᵢ(x) = βᵢ · zᵢ(x)` and `E[f(x)] = intercept`. The unit test `test_linear_shap_sums_to_log_odds` proves the reconstruction is exact. |
...
> *The 18 % error rate is not a failure. It is the share of passengers whose individual humanity exceeded the reach of demographic statistics. Thomas Andrews is in that 18 %. My job as an engineer is to measure it, report it honestly, and resist the temptation to add another feature to make it smaller.*
